...
register 	uchar mask_bit  asm("r10");
register 	uchar tdi_byte  asm("r11");
register 	uchar tdo_byte  asm("r12");
register 	uchar bitcnt    asm("r13");
//register	uchar 	jcmd	asm("r14");						// jtag command
//register	uchar 	maxcnt	asm("r15");
#else
...
//	PORTB PIN ASSIGN
#define	ISP_SCK		7			/* PB7 Target SCK */
#define	ISP_MOSI	6			/* PB6 Target MOSI (tn2313:MISO) */
#define	ISP_MISO	5			/* PB5 Target MISO (tn2313:MOSI) */
#define	ISP_RST		4			/* PB4 Target RESET */
#define	ISP_RDY		3			/* PB3 Green  LED */
#define	ISP_RED		2			/* PB2 RED LED */

//	PORTB ALIAS NAME(JTAG)
#define	TCK			7			/* PB7 (Target SCK  ) */
#define	TDI			6			/* PB6 (Target MOSI ) */
#define	TDO			5			/* PB5 (Target MISO ) */
#define	TMS			4			/* PB4 (Target RESET) */
#define	ISP_IN		PINB
...
#if	INCLUDE_JTAG_CMD
static inline void set_tms(int tms)
{
	if(tms) {
...
void jtag_command()
{
	// report.buf は読み書き共用で、TDO読み取り結果を1byte手前のストリームに上書き.
	uchar *p = report.buf + 1;		// jtag ストリーム(for WRITE)
	uchar *q = report.buf;			// jtag ストリーム(for READ)
