...
#if TEST_MODE	// usbdrv側でレジスタが使用されていないことを条件に割り付ける
	#define OPTIMIZE_SIZE		0	// r4,r5,r6,r7,r8を global変数として使用しない
#else
...
#if OPTIMIZE_SIZE
	register addr_t			currentAddress asm("r4");	/* in bytes	*/
	register uchar 			*page_addr 	asm("r6");
...
int main(void)
{
	...
	while(1) { /* nothing */ }	// omit return process
}
