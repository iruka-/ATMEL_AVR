...
#else
	#include "../firmware/hidcmd.h"	// HIDASP_BOOT_EXIT

	#define IDENT_VENDOR_STRING     "AVRetc"
	#define IDENT_PRODUCT_STRING    "bmon"
#endif
...
static int parseIntelHex(char *hexfile, char buffer[65536 + 256], int *startAddr, int *endAddr)
{
	...
	while (parseUntilColon(input) == ':') {
		...
		if ((sum & 0xff) != 0) {
			fprintf(stderr,
					"Warning: Checksum error between address 0x%x and 0x%x\n",
					base, address);
		}
		if (*startAddr > base)
			*startAddr = base;
...
static int uploadData(char *dataBuffer, int startAddr, int endAddr)
{
	...
	err = usbGetReport(dev, USB_HID_REPORT_TYPE_FEATURE, 1, buffer.bytes, &len);
	if (err != 0) {
	...
	printf("Device size = %d (0x%x); %d bytes remaining\n", deviceSize, deviceSize, deviceSize - BOOT_SIZE);

	if (endAddr > deviceSize - BOOT_SIZE) {
	...
	while (startAddr < endAddr) {
		...
		err = usbSetReport(dev, USB_HID_REPORT_TYPE_FEATURE, buffer.bytes, sizeof(buffer.data));
		if (err != 0) {
...
#if	HIDASP_BOOT_EXIT
		buffer.info.reportId = 1;
		buffer.info.pageSize[0] = HIDASP_BOOT_EXIT;	// command
		buffer.info.pageSize[1] = 0;
#endif
